...
// Gators Sports Bar and Grill, Kent WA. Hardcoded Place ID for V1.
// Looked up via Google Places "Find Place" search; pinned here so we
// know we are getting the right Gators across multiple results nationally.
const GATORS_PLACE_ID = 'ChIJN1t_tDeuEmsRUsoyG83frY4'; // placeholder, will resolve at runtime if invalid
...
export async function GET() {
  ...
  // Step 1: if the hardcoded Place ID is wrong, find Gators Kent via text search.
  // We do this defensively so the route works even before we manually pin the right ID.
  ...
  // Step 2: fetch full details for that place.
