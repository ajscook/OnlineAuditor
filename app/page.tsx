...
const initialResult = (strategy: Strategy): PSIResult => ({
  strategy,
  loading: false,
  error: null,
  data: null,
...
export default function Home() {
  const [mobile, setMobile] = useState<PSIResult>(initialResult('mobile'));
  const [desktop, setDesktop] = useState<PSIResult>(initialResult('desktop'));
  const [running, setRunning] = useState(false);

  async function runAudit() {
    if (!API_KEY) {
      alert('NEXT_PUBLIC_GOOGLE_API_KEY is missing. Add it to .env.local and restart the dev server.');
      return;
    }
    setRunning(true);
    setMobile({ ...initialResult('mobile'), loading: true });
    setDesktop({ ...initialResult('desktop'), loading: true });
    ...
    await Promise.all([mobileJob, desktopJob]);
    setRunning(false);
  }
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: 22, marginBottom: 8 }}>PSI Smoke Test</h1>
      ...
      >
        {running ? 'Running PageSpeed...' : 'Run PageSpeed Audit'}
      </button>

      <ResultCard result={mobile} />
      <ResultCard result={desktop} />
    </main>
  );
}

function ResultCard({ result }: { result: PSIResult }) {
  return (
    <section
      style={{
    ...
    >
      <h2 style={{ textTransform: 'capitalize', marginTop: 0, marginBottom: 12, fontSize: 16 }}>
        {result.strategy}
      </h2>
